...
<div class="card">
  ...
  <div class="card-body">
    <form method="POST" action="{{ route("entities.update", [$entity->id]) }}" enctype="multipart/form-data">
      ...
      <div class="form-group">
        <label class="required" for="label">{{ trans('cruds.entity.fields.label') }}</label>
        <input class="form-control {{ $errors->has('label') ? 'is-invalid' : '' }}" type="text" name="label" id="label" value="{{ old('label', $entity->label) }}" required>
        @if($errors->has('label'))
        <span class="text-danger">{{ $errors->first('label') }}</span>
        @endif
        <span class="help-block">{{ trans('cruds.entity.fields.label_helper') }}</span>
      </div>
      <div class="form-group">
